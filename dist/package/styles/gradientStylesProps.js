...
var gradientStylesProps = function (props) {
    ...
    if (type === "linear") {
        background = "linear-gradient(".concat(degree, "deg, ").concat(colors
            .map(function (c) { return "".concat(c.color, " ").concat(c.stop ? c.stop + "%" : ""); })
            .join(", "), ")");
    }
    else if (type === "radial") {
        background = "radial-gradient(".concat(colors
            .map(function (c) { return "".concat(c.color, " ").concat(c.stop ? c.stop + "%" : ""); })
            .join(", "), ")");
    }
    return {
